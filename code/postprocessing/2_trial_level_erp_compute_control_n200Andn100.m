...
output_location = [main_dir filesep 'derivatives' filesep 'eeg' filesep 'preprocessed' filesep 'processed_data'];
...
for s = 1:length(sub_file_name_list)
    ...
    % ern, 801~901 (-200ms ~ -100ms)
    cur_ern_ched = squeeze(mean(cur_resp_all_corrected(chn_cluster_ern,:,:),1));
    cur_ern_avg = squeeze(mean(cur_ern_ched(801:901, :),1));
    ...
    num_all_list = [num_all_list {EEG.event.num_all}];
    num_block_list = [num_block_list {EEG.event.num_block}];
    num_inblock_list = [num_inblock_list {EEG.event.num_inblock}];
...
various = {'subject', 'erp_ern', 'erp_pe400', 'erp_pe500', ...
    ...
    'nextValidRt', 'nextValidTrial', 'num_all', 'num_block', 'num_inblock'};
result_table = table(subj_list',erp_ern',erp_pe400_list',erp_pe500_list',observation_list', ...
    ...
    nextAccuracy_list', nextRt_list', nextValidRt_list', nextValidTrial_list', ...
    num_all_list', num_block_list', num_inblock_list', 'VariableNames',various);
writetable(result_table, 'trial_erp_all_control_n200And100_with_trialNO.csv');
